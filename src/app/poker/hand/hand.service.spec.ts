...
describe('handExtractor.getBest', () => {
  ...
  it('givenEmpty_whenGetBest_thenUndefined', () => expect(getBestCards(empty)).toBeUndefined());
  it('givenEightWithInterlopingFullHouses_whenGetBest_thenFullHouse', () =>
    expect(getBestCards(eightWithInterlopingFullHouses).map(card => card.rank))
      .toEqual([1, 1, 1, 6, 6]));
  it('givenFiveWithTwoPair_whenGetBest_thenTwoPair', () =>
    expect(getBestCards(fiveWithTwoPair).map(card => card.rank))
      .toEqual([1, 1, 6, 6]));
  ...
  it('givenSevenWithFiveOfAKind_whenGetBest_thenRoyalStraight', () =>
    expect(getBestCards(sevenWithFiveOfAKind).map(card => card.rank))
      .toEqual([4, 4, 4, 4, 4]));
